...
def solution(a)
  # construct prefix sum array for westward cars
  prefix_sums = [a[0]]
  (1...a.length).each do |idx|
    prefix_sums[idx] = prefix_sums[idx - 1] + a[idx]
  end

  max_west_cars = prefix_sums.last
  total_passing_cars = 0
  a.each_with_index do |car, idx|
    next unless car == 0 # only consider eastward cars
    current_passing_cars = max_west_cars - prefix_sums[idx]
    total_passing_cars += current_passing_cars
    return -1 if total_passing_cars > 1000000000
  end
