...
def solution(x, a)
  triangle_number = (1..x).reduce { |sum, num| sum += num }
  seen_positions = {}
  current_sum = 0

  a.each_with_index do |position, second|
    next if seen_positions[position] || position > x

    current_sum += position
    seen_positions[position] = true

    return second if current_sum == triangle_number
  end
