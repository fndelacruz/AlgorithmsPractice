...
def solution(a)
  # immediately handle minimal size array
  return a.reduce { |prod, num| prod * num } if a.size == 3

  a_sorted = a.sort
  left_product = a_sorted[0] * a_sorted[1] * a_sorted[-1]
  right_product = a_sorted[-3] * a_sorted[-2] * a_sorted[-1]
  [left_product, right_product].max
end
