...
def solution(a)
  ...
  p_sums = [0] + Array.new(a.size)
  a.each_with_index { |el, idx| p_sums[idx + 1] = p_sums[idx] + el }

  min_start = nil
  min_avg = nil

  # only consider slices of size 2 or 3
  (2..3).each do |slice_size|
    # puts "slice: #{slice_size}"
    (0..(a.size - slice_size)).each do |slice_start|
      current_avg =
        (p_sums[slice_start + slice_size] - p_sums[slice_start]) / slice_size.to_f
      # puts "min_start: #{min_start}, min_avg: #{min_avg}, slice_start: #{slice_start}, current_avg: #{current_avg}, "
      if !min_start || current_avg < min_avg
        min_avg = current_avg
        min_start = slice_start
      end
    end
  end
